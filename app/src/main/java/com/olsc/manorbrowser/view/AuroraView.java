 ...
 * 动态背景视图 - 极光效果
 * 此类实现了两种模式：
 * 1. 极光模式：模拟带有深色夜空、闪烁星空和流动的极光波幔 (AuroraCurtain)。
 * 2. 纯色模式：支持通过交互式色盘 (ColorWheel) 选择全局背景色。
 * 核心技术：
 * - 极光：结合三层正弦波合成复杂的动态波形。
 * - 毛玻璃：使用 BlurMaskFilter 实现极光的边缘虚化。
 * - 交互：自定义触控色盘逻辑。
 */
